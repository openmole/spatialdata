...
case class ShortestPathsNetworkLoader(
  ...
  override def load(odPattern: Option[Map[(Node, Node), Double]]): NetworkLoading =
...
object ShortestPathsNetworkLoader {
  ...
  def load(network: Network, odPattern: Option[Map[(Node, Node), Double]], pathSample: Double): NetworkLoading = {
    ...
    val nwWithPaths = network.computeShortestPaths(l=> l.weight*l.length)
