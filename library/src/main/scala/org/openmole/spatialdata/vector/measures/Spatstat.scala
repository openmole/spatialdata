...
/**
  ...
  *
  * Possible extensions:
  *   - add standard tests / spatial point processes
  *   - Implement Rey, S. J., & Janikas, M. V. (2006). STARS: space–time analysis of regional systems. Geographical analysis, 38(1), 67-86.
  *
  */
object Spatstat {
  ...
  def spatialMoment(pi: Array[Point],x: Array[Double],p: Int = 0,q: Int = 0,filter: Double => Boolean = _ => true): Double = {
    ...
    val mx = centroid._1
    val xnorm = pf.map(point => (point._1 - mx) / sx)
    ...
    val my = centroid._1
    val ynorm = pf.map(point => (point._2 - my) / sy)
  ...
  def moran(pi: Array[Point],x: Array[Double],weightFunction: Array[Point]=> Array[Array[Double]] = spatialWeights,filter: Double => Boolean = _ => true): Double = {
    val (pf,xf) = pi.zip(x).filter{case (_,xval)=>filter(xval)}.unzip
    val n = pf.length
    ...
    val variance = xx.map(xi => xi*xi).sum
  ...
  /**
    ...
    * @param x field values
    */
  def moran(pi: Array[Array[Double]],x: Array[Double]): Double = moran(pi.map(a => (a(0),a(1))),x)
  ...
  def averageDistance(pi: Array[Point],x: Array[Double],filter: Double => Boolean = _ => true): Double = {
    val (pf,xf): (Array[Point],Array[Double]) = pi.zip(x).filter{case (_,xx)=>filter(xx)}.unzip
    val n = pf.length
    ...
    xi.zip(xj).zip(dmat.flatten.toSeq).map{case ((xival,xjval),dij)=>xival*xjval*dij}.sum / (xtot*xtot*dmax)
  }
  ...
  def euclidianDistanceMatrix(pi: Array[Point], pj: Array[Point]): Array[Array[Double]] = {
    ...
    MatrixUtils.createRealMatrix(
      xi.subtract(xj).getData.map(_.map(x => x*x))
    ).add(
      MatrixUtils.createRealMatrix(yi.subtract(yj.transpose()).getData.map(_.map(x => x*x)))
    ).getData.map{_.map{math.sqrt}}
  }
  ...
  /**
    ...
    *
    *  Rq:
    *   - for scalar summary of ripley, instead of piecewise linear, use integral ?
    *   - add generic edge correction implementation?
    *
  ...
  /**
    ...
    *
    * @param pi points
    * @param radiusSamples number of samples for the radius
    * @param radiusValues alternative sampling function for radius values
    * @return
    */
