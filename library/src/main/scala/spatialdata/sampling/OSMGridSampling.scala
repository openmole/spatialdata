...
object OSMGridSampling {
  ...
  def sampleGridsInLayer(layer: String,nPoints: Int,windowSize: Double,worldWidth: Int,weightAttribute: String = "")(implicit rng: Random): Seq[RasterLayerData[Double]] = {
    ...
    trPoints.map{case (lon,lat) => OSMGridGenerator(lon,lat,windowSize,worldWidth).generateGrid}
  }
